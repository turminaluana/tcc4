...
function AdminAnimais() {
  ...
  async function carregarAnimais() {
    try {
      setCarregando(true);
      // admin=true traz todos os animais (disponíveis, indisponíveis e adotados)
      ...
      {!carregando && animais.length > 0 && (
        <div className="lista-admin-animais">
          {animais.map((animal) => {
            ...
            const foiAdotado = !animal.disponivel && Boolean(animal.adotadoPor);
            ...
            return (
              ...
              >
                  ...
                  <div className="animal-admin-info">
                    ...
                    <p style={{ margin: "2px 0" }}>
                      ...
                      {animal.disponivel ? (
                        ...
                        <span style={{ color: "#d32f2f", fontWeight: "bold" }}>Adotado</span>
                      ) : (
                        <span style={{ color: "#d32f2f", fontWeight: "bold" }}>Não</span>
                      )}
                    </p>

                    {/* Exibe o adotante APENAS se tiver sido adotado */}
                    {foiAdotado && (
                      <div style={{ marginTop: "6px", padding: "6px", backgroundColor: "#f0f4f8", borderRadius: "4px" }}>
                        <p style={{ margin: "0", fontSize: "0.85rem", color: "#333" }}>
                          <strong>Adotado por:</strong> {animal.adotadoPor?.nome || "Não informado"}
                        </p>
                        {animal.dataAdocao && (
                          <p style={{ margin: "2px 0 0 0", fontSize: "0.8rem", color: "#666" }}>
                            <strong>Data:</strong> {new Date(animal.dataAdocao).toLocaleDateString("pt-BR")}
                          </p>
                        )}
                      </div>
                    )}
                ...
                {/* Permite Editar/Excluir se o animal estiver disponível OU apenas desativado manualmente (NÃO adotado) */}
                {!foiAdotado && (
